...
async function confirm(rl, summary) {
  return new Promise((resolve) => {
    console.log();
    console.log(import_chalk2.default.yellow("  -- confirm transaction --"));
    console.log(import_chalk2.default.dim(summary));
    console.log();
    rl.question(import_chalk2.default.bold('  Type "yes" to proceed: '), (ans) => {
      console.log();
...
async function dispatchTool(name, args, rl) {
  ...
  Wallet: ${walletName} (${w.address})`;
        ...
        if (q.approvalAddress) {
          console.log(import_chalk2.default.dim("  checking token approval..."));
        ...
        console.log(import_chalk2.default.dim("  submitting transaction..."));
    ...
    try {
      ...
      while (message.tool_calls && message.tool_calls.length > 0) {
        if (++toolIterations > 10) {
          console.log(import_chalk2.default.yellow("  too many tool calls \u2014 stopping loop"));
          break;
        }
        ...
        for (const call2 of message.tool_calls) {
          ...
          console.log(import_chalk2.default.dim(`  [tool] ${call2.function.name}(${JSON.stringify(args)})`));
      ...
      console.log(import_chalk2.default.green("agent> ") + formatted);
      console.log();
    } catch (err) {
      const msg = String(err);
      messages.pop();
      if (msg.includes("429")) {
        console.log(import_chalk2.default.yellow("  rate limited \u2014 wait a moment and try again"));
        console.log(import_chalk2.default.dim(`  or switch model: lifi-cli agent --model meta-llama/llama-3.3-70b-instruct:free`));
      } else if (msg.includes("401") || msg.includes("403")) {
        console.log(import_chalk2.default.red("  auth error \u2014 check your API key: lifi-cli agent --setup"));
      } else {
        console.log(import_chalk2.default.red("  error: ") + msg);
      }
      console.log();
    }
  }
